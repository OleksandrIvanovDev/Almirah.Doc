...
- **A cross-record dependency graph**: `depends_on`, with a record's deliverable defined by its `Code`/`Tests` rows ([[adr-194-full-kit-readiness]]). These are the inter-record edges.
- **A single owner per row**: each Scope row carries one `Owner` ([[adr-193-owner-wip-heatmap]]). This is what creates resource contention — rows sharing an owner cannot run at once — which is precisely what makes a *critical chain* differ from a plain critical path.
...
3. **Cross-record edges**: for each `depends_on` prerequisite of a record, that record's **first-step** rows follow the prerequisite's **deliverable rows** (its `Code`/`Tests` rows, or all rows if it has neither — the same rows that define satisfaction in [[adr-194-full-kit-readiness]]). Prerequisites in another release are treated as already-available inputs, not scheduled nodes.
4. **Resource levelling.** Schedule with a deterministic greedy heuristic: process rows in priority order (longest downstream-path duration first; ties broken by record sequence number, then step number) and start each at the earliest time at which all its predecessor rows have finished **and** its owner is free. Rows sharing an owner therefore serialise even with no edge between them.
...
- **Per-row cross-record edges.** A cross-record dependency attaches at the dependent record's first step, not at an arbitrary downstream row; finer wiring is deferred.
...
| 3 | For each Target Release Version, the software shall construct a planning network whose nodes are the not-Done Scope rows of the records targeting that release, with intra-record edges following the step-number order and cross-record edges placing a dependent record's first-step rows after each prerequisite's deliverable rows. | >[SRS-122] |
| 4 | The software shall schedule the planning network with a deterministic resource-levelling rule in which each row starts only when all its predecessor rows have finished and its owner is free, so that rows sharing an owner do not run concurrently. | >[SRS-123] |
...
- [[adr-194-full-kit-readiness]] — supplies the `#` step ordering and the `depends_on` deliverable graph the chain runs through; this record's own prerequisite
