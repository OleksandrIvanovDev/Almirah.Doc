...
|---|---|---|---|---|---|---|---|---|---|
| 1 | Code | DEV |  |  |  | Implemented | 07-06-2026 | 07-06-2026 | In `lib/almirah/templates/css/main.css`, replace `#nav_pane`'s `height: 100%` with `top: 0; bottom: 0;` so the fixed pane spans exactly the viewport and its internal scroll reaches the last item |
| 2 | Requirements | BA |  |  |  | Implemented | 07-06-2026 | 07-06-2026 | Authored SRS-101 (under the "Navigation" subsection of the User Interface chapter in `srs.md`) specifying that the navigation pane scrolls so every sections-tree item can be brought fully into view; recorded in this issue's Affected Documents section |
| 3 | Tests | TEST |  |  |  | Implemented | 07-06-2026 | 07-06-2026 | Automated end-to-end browser test (`spec/e2e/nav_pane_scroll_spec.rb`) that renders a tall sections tree, opens the pane, scrolls it to the bottom in headless Chrome, and asserts the last item is fully within the viewport; traced to SRS-101. Verified to fail without the fix (last item bottom 534px vs viewport 513px) and pass with it |
